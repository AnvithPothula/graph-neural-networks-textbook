...
class DiagramAnalyzer:
    ...
    # Causal-loop / feedback diagram keywords → classify as 'diagram' not 'microsim'
    CLD_KEYWORDS = [
        'causal loop diagram', 'feedback loop', 'feedback dynamics',
        'causal loop', 'reinforcing loop', 'balancing loop',
    ]

    def __init__(self, chapters_dir: str, verbose: bool = False):
        self.chapters_dir = Path(chapters_dir)
        # sims dir lives alongside docs/chapters/ → up one level from chapters_dir
        self.sims_dir = Path(chapters_dir).parent / 'sims'
        self.elements: List[VisualElement] = []
        self.verbose = verbose

    def _sim_info_from_metadata(self, sim_id: str) -> dict:
        """Return bloom, framework, learning_objective from a sim's metadata.json."""
        meta_path = self.sims_dir / sim_id / 'metadata.json'
        if not meta_path.exists():
            return {}
        try:
            import json
            data = json.loads(meta_path.read_text())
            edu = data.get('educational', {})
            return {
                'bloom': edu.get('bloomsTaxonomy', ''),
                'framework': data.get('technical', {}).get('framework', ''),
                'learning_objective': (edu.get('learningObjectives') or [''])[0],
            }
        except Exception:
            return {}

    def _count_ui_from_js(self, sim_id: str) -> int:
        """Count p5.js UI-creation calls in the sim's JS file."""
        js_pattern = re.compile(
            r'\b(createSlider|createButton|createSelect|createCheckbox|createInput|createRadio)\b',
            re.IGNORECASE
        )
        js_dir = self.sims_dir / sim_id
        count = 0
        if js_dir.exists():
            for js_file in js_dir.glob('*.js'):
                try:
                    count += len(js_pattern.findall(js_file.read_text()))
                except Exception:
                    pass
        return count

    def _classify_type(self, title: str, sim_meta: dict) -> str:
        """Return 'diagram' for causal-loop / concept-map content, 'microsim' otherwise."""
        title_lower = title.lower()
        for kw in self.CLD_KEYWORDS:
            if kw in title_lower:
                return 'diagram'
        if 'concept map' in title_lower or 'concept-map' in title_lower:
            return 'diagram'
        return 'microsim'
    ...
    def analyze_chapter_file(self, file_path: Path):
        ...
        try:
            ...
            # Primary: #### Diagram: header + <details> block (full spec format)
            ...
            # Fallback: #### Diagram: header followed by iframe (inline format — no <details>)
            # Match: #### Diagram: Title\n\n...<iframe src="...sims/SIM-ID/main.html"...>
            if not elements_found:
                inline_pattern = re.compile(
                    r'####\s+Diagram:\s*([^\n]+)\n+(?:[^\n<].*\n)*?'
                    r'.*?<iframe[^>]+src=["\'](?:[^"\']*sims/([^/]+)/main\.html)["\'][^>]*>',
                    re.DOTALL
                )
                for m in inline_pattern.finditer(content):
                    title = m.group(1).strip()
                    sim_id = m.group(2).strip() if m.group(2) else ''
                    meta = self._sim_info_from_metadata(sim_id)
                    ui_count = self._count_ui_from_js(sim_id)
                    elem_type = self._classify_type(title, meta)
                    bloom = [meta['bloom']] if meta.get('bloom') else ['Not specified']
                    element = VisualElement(
                        ...
                        estimated_difficulty=self.estimate_difficulty('', ui_count, elem_type),
                        status='Implemented',
                        learning_objective=meta.get('learning_objective', ''),
                        specifications=sim_id,
                    )
                    self.elements.append(element)
                    elements_found += 1
                    if self.verbose:
                        print(f"      Inline sim: {title} ({sim_id}) bloom={bloom} ui={ui_count} type={elem_type}")

            # Second fallback: detect iframes with sims/ paths not already captured
            # (for chapters that use ## N.N MicroSim: headers without #### Diagram:)
            found_sim_ids = {e.specifications for e in self.elements
                             if e.chapter_num == chapter_num}
            iframe_pattern = re.compile(
                r'<iframe[^>]+src=["\'][^"\']*sims/([^/]+)/main\.html["\'][^>]*>',
                re.IGNORECASE
            )
            # Build map of sim_id → nearest preceding header text
            header_pattern = re.compile(r'^#{1,4}\s+(?:\d[\d.]*\s+)?(?:MicroSim:\s*)?(.+)$', re.MULTILINE)
            headers = [(m.start(), m.group(1).strip()) for m in header_pattern.finditer(content)]

            for m in iframe_pattern.finditer(content):
                sim_id = m.group(1).strip()
                if sim_id in found_sim_ids:
                    continue
                # Find the nearest preceding header
                pos = m.start()
                title = sim_id  # fallback
                for hpos, htitle in reversed(headers):
                    if hpos < pos:
                        title = htitle
                        break
                # Strip leading chapter numbering like "11.12 "
                ...
                self.elements.append(element)
                found_sim_ids.add(sim_id)
                elements_found += 1
                if self.verbose:
                    print(f"      Fallback sim: {title} ({sim_id}) bloom={bloom} ui={ui_count} type={elem_type}")
